...
public class HalfFactory :BaseFactory {
    ...
    public HalfFactory(int numPerRow,int blackNum)
    {
    ...
    public override void BuildRowBlocks(out GameObject[] objs)
    {
        ...
        for (int i = 0; i < objs.Length;i++ )
        {
            ...
            {
                objs[arr[i]] = GameObject.Instantiate<GameObject>(block);
                objs[arr[i]].GetComponent<BaseBlock>().Init(whliteSprite, whliteDownSprite, new WhileClick_subScore(objs[arr[i]]), objs.Length);
